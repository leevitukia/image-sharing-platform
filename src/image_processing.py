...
def createThumbnail(image: FileStorage) -> bytes | None:
    tempFile = f"{''.join(random.choices(string.ascii_uppercase, k=40))}.avif"
    fileBytes: bytes = image.stream.read()
    resolution = getImageResolution(fileBytes)
    if(resolution == None or resolution[0] > 8192 or resolution[1] > 8192):
    ...
    if(resolution[0] > resolution[1]):
        cropFilter = f" -vf \"crop=ih:ih:{(resolution[0] - resolution[1]) / 2}:0, scale=256:256:flags=lanczos\""
    elif(resolution[1] > resolution[0]):
        cropFilter = f" -vf \"crop=iw:iw:0:{(resolution[1] - resolution[0]) / 2}, scale=256:256:flags=lanczos\""    
